...
/**
 * Created by kenneth on 7/30/16.
 */
public class JsonRpcHttpClientWithRSA extends JsonRpcClient implements IJsonRpcClient {
     ...
     */
    public JsonRpcHttpClientWithRSA(URL serviceUrl, Map<String, String> headers) {
     ...
     * @param headers the headers
     */
    public JsonRpcHttpClientWithRSA(ObjectMapper mapper, URL serviceUrl, Map<String, String> headers) {
        super(mapper);
     ...
     * @param serviceUrl the service end-point URL
     */
    public JsonRpcHttpClientWithRSA(URL serviceUrl) {
    ...
    public Object invoke(String methodName, Object argument, Type returnType, Map<String, String> extraHeaders) throws Throwable {
        ...
        connection.connect();
        try {
            try (OutputStream send = new RSAOutputStream(connection.getOutputStream())) {
                super.invoke(methodName, argument, send);
            }
            final boolean useGzip = useGzip(connection);
            // read and return value
            try {
                try (InputStream answer = new RSAInputStream(getStream(connection.getInputStream(), useGzip))) {
